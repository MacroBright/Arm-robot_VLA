...
class ZdtConfig:
    channel: str = "can0"
    bitrate: int = 500_000
    # 读回帧超时预算 (秒). 2026-08-30 真机日志: Emm_V5 驱动回 0x36/0x27/0x3A
    # 的延迟贴着 ~30ms 浮动 (attempt0 超时、attempt1 才回), 0.03s 预算偏紧导致
    # 告警风暴; 读已移出主循环 (后台 25Hz 轮询), 放宽到 0.10s 只影响后台线程,
    # 不再有卡主循环风险. 若 0.10s 下仍偶发超时 → 属单驱动器回帧故障, 需延迟探针定位.
