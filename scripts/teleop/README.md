...
3. **推拿专属 3 大姿态解耦模态（Tuina Task Modes）**：
   - **模式 1: 垂直点按揉捏模式 (`KNEAD` / 姿态锁定)**：末端姿态强制垂直朝下锁定（$\vec{\omega}=0$），空间移动自由，手指任意进行 16-DOF 揉捏/点按，末端姿态 100% 稳如泰山！
   - **模式 2: 滚法一指禅推法模式 (`ROLL_ONLY` / 单轴滚转)**：锁定 Pitch/Yaw，仅开放沿臂前进轴的 Roll 滚转，专为推拿“滚法”标准动作设计；
   - **模式 3: 全 6-DOF 自由遥操模式 (`FULL_6DOF`)**：开放 3 轴全部自由度，满足复杂手法需求。
   - **按键支持**：在运行时随时按键盘 **`M`**（循环切换）或 **`1` / `2` / `3`** 直达对应模式。
4. **线速度平滑与低速安全透传**：
