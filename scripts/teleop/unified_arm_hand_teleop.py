...
def _draw_unified_dashboard(frame: np.ndarray,
    ...
    """在 1280x720 画面上绘制紧凑型高透明度 HUD 看板 (左上姿态看板缩减1/3, 右上电机看板缩减1/3, 灵巧手看板移至左下并缩小30%)."""
    h, w = frame.shape[:2]
    ...
    # 左上角机械臂动作姿态看板 (缩小至 260px, 原 390px 的 2/3)
    lx0, ly0 = 14, 52
    l_box_w, l_box_h = 260, 135
    cv2.rectangle(overlay, (lx0, ly0), (lx0 + l_box_w, ly0 + l_box_h), (16, 18, 24), -1)

    # 右上角机械臂电机状态看板 (缩小至 220px, 原 330px 的 2/3)
    box_w = 220
    rx0, ry0 = w - box_w - 14, 52
    box_h = 135
    cv2.rectangle(overlay, (rx0, ry0), (rx0 + box_w, ry0 + box_h), (16, 18, 24), -1)

    # 左下角灵巧手舵机看板 (移至左下, 等比缩小 30%: 260x170px)
    hx0, hy0 = 14, h - 30 - 175
    ...
    # 机械臂模式
    cv2.putText(frame, f"ARM: {MODE_NAMES[arm_mode]}", (155, 27), cv2.FONT_HERSHEY_SIMPLEX, 0.52, (0, 220, 255), 2)
