...
void MX_USART3_UART_Init(void)
{
  ...
  HAL_UART_Receive_IT(&huart3, (uint8_t *)uart3_rx_buff, 1); // 启动串口接收中断
	...
	if (huart->Instance == USART1) {
		...
		HAL_UART_Receive_IT(&huart1, (uint8_t *)&uart1_rx_buff[uart1_rx_pos], 1); // 重新启动串口接收中断
	       ...
	       if (uart3_rx_buff[uart3_rx_pos] == '\n') {
	           if (uart3_rx_pos > 0 && uart3_rx_buff[uart3_rx_pos - 1] == '\r') {
	               uart3_rx_buff[uart3_rx_pos - 1] = '\0';
	           }
	           uart3_rx_buff[uart3_rx_pos] = '\0';
	           robot_cmd_send_from_isr((char *)uart3_rx_buff, CMD_TYPE_BLE);
	           memset(uart3_rx_buff, 0, uart3_rx_pos + 1);
	           uart3_rx_pos = 0;
	       } else {
	           uart3_rx_pos += 1;
	       }
