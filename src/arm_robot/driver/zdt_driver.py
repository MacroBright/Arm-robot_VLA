...
class ZdtDriver:
    ...
    def __init__(self, transport: CanTransport, timeout_s: float = 0.1,
        ...
        # 读事务串行锁: 后台状态轮询线程与主线程 (宏回读/启动) 会并发调用本层.
        # 不串行化时两线程并发 recv() 会互相消费对方回帧 (response stealing) →
        # 伪超时 / get_real_state 降级 / 宏动作 TimeoutError 崩溃.
        # 只锁 expect_response 的读事务; fire-and-forget (急停/运动) 不取锁,
        # 保持 e_stop 任意时刻零延迟广播 (zdt_bus.py 同设计目标).
    ...
    def _request(self, addr: int, body: bytes,
        ...
        if not expect_response:
            # fire-and-forget (急停/运动/多机同步): 不加锁, 保持 e_stop 零延迟
            for f in frames:
                try:
                    self._t.send(f)
        ...
        # 读事务: 发送+等回帧必须原子 (多线程并发 recv 会互相偷吃回帧).
