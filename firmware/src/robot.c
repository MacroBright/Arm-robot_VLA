...
static void robot_control_task(void *arg)
{
	...
	// 主循环：带超时等待事件，超时后刷新角度
    while(1) {
        if (xQueueReceive(g_robot.event_queue, &event, pdMS_TO_TICKS(50)) == pdPASS) {
            switch (event.type) {
...
void robot_cmd_service(void)
{
	...
	while(xQueueReceive(g_robot.cmd_queue, &rb_cmd, portMAX_DELAY) == pdPASS) {
		switch (rb_cmd.type)
		{
			...
			case CMD_TYPE_BLE:
				LOG("[BLE] raw: %s\n", rb_cmd.cmd);
				robot_uart1_handle(&rb_cmd);  /* 蓝牙命令复用 UART1 */
				break;
...
static int robot_update_current_angle(uint8_t joint_id)
{
	...
	while(!can.rxFrameFlag) {
		if ((HAL_GetTick() - start_tick) > ROBOT_CAN_TIMEOUT) {
			/* 首次超时打一次日志, 之后静默避免刷屏 */
			if (!ROBOT_STATUS_IS(joint->status, 7)) {
				LOG("joint %u update current angle timeout.\n", joint_id);
				ROBOT_STATUS_SET(joint->status, 7);
			}
			xTaskResumeAll();
			return 1;
		}
	...
	/* CAN 恢复后清除超时标记 */
	ROBOT_STATUS_CLEAR(joint->status, 7);
	taskENTER_CRITICAL();
	xTaskResumeAll();
