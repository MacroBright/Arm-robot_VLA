...
from lerobot_robot_massage.zdt.frames import add_checksum
...
from lerobot_robot_massage.zdt.scan import scan_bus
...
def dump_mode(bus: ZdtBus, sm: SafetyMachine, args) -> None:
    ...
    if args.probe is not None:
        from lerobot_robot_massage.zdt.scan import probe_id
        cid = args.probe
        m = probe_id(bus, cid)
        if m is None:
    ...
    # 只读遥测 (Phase 2 全表)
    from lerobot_robot_massage.zdt.scan import read_telemetry
...
def main() -> int:
    ...
    from lerobot_robot_massage.zdt.can_transport import SocketCanTransport
