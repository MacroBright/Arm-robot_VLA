"""Arm-robot_VLA.scripts.teleop.test_teleop_config — 集中配置文件单元测试 (包含关节限位验证)."""
import tempfile
from pathlib import Path
import pytest

from arm_robot.teleop.teleop_config import (
    TeleopConfig,
    GearConfig,
    JointFactorConfig,
    JointLimitsConfig,
...
def test_teleop_config_load_yaml():
    yaml_path = Path(__file__).resolve().parents[2] / "configs" / "teleop_config.yaml"
    if yaml_path.exists():
        cfg = TeleopConfig.load(yaml_path)
        assert cfg.gear.default_gear == 2
        assert cfg.joint_factor.j5_wrist_pitch in (1.0, 1.5)
        assert cfg.joint_factor.j1_base_yaw in (2.0, 2.5)
        assert cfg.joint_factor.j4_wrist_roll_1 == 1.5
        assert cfg.joint_limits.j1_base_yaw in ([-1.0, 360.0], [-90.0, 270.0])
        assert cfg.joint_limits.j2_shoulder_pitch in ([-1.0, 150.0], [-10.0, 150.0])
        assert cfg.joint_limits.joint_limit_margin_deg == 2.0
        assert cfg.motor.speed_rpm in (1500.0, 2000.0)
        assert cfg.gear.gear_2_mid.lin_scale in (0.080, 0.60)
        assert cfg.gear.gear_3_high.lin_scale in (0.085, 0.95)
...
def test_teleop_config_validation_safety():
    ...
    cfg.joint_limits.j2_shoulder_pitch = [0.0, 50.0]
