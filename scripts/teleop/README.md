...
在弹出的 OpenCV 画面窗口中，支持以下键盘交互：

- **`H` 键 (Clutch 离合器)**：
  - 按住或切换：暂停/重新锚定当前手部参考点。当人手移出视野或需要大幅调整起始位置时使用。
- **`R` 键 (Ready / Reset 复位)**：
  - 机械臂平滑回到预设安全初始姿态（`READY_POSE`）。
- **`Y` 键 (E-Stop 紧急制动)**：
  - 立即向 CAN 总线广播 `0x0000 0xFE` 抱闸停机，状态机切入 `STOPPED`。
- **`Q` 键 / `ESC` 键 (安全退出)**：
  - 停止发送速度，平稳断开连接并保存当前录制数据后退出。
